namespace StarterProject.Web.Api.Controllers
{
    ...
    public class GameController : Controller
    {
        ...
        public string Index([FromForm]string map)
        {
            ...
            Point nextPoint = (goingHome) ? GoToDestination(gameInfo.Player.HouseLocation) : GoToDestination(closestResource);
            string action = AIHelper.CreateMoveAction(nextPoint);

          

            if (IsNearTile(closestResource) && !goingHome)
            {
                if (gameInfo.Player.CarriedResources < gameInfo.Player.CarryingCapacity)
                {
            ...
            if (goingHome)
            {
                if (IsSameTile(gameInfo.Player.HouseLocation))
                {
                    ...
                    // action = AIHelper.CreateMoveAction(nextPoint = new Point(29,gameInfo.Player.Position.Y-1));
        ...
        {
            return (GoToDestination(new Point(destination.X, destination.Y)));
        }
